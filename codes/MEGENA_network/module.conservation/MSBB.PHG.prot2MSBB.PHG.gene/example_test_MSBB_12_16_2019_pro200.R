...
####################
wd <- "/sc/hydra/projects/zhangb03a/shared/DET_WINA_MEGENA/PHG_MEGENA_module_conservation"
setwd(wd)
out.dir <- "module_preservation_by_WGCNA";
outfname <- "TCGA_GCC.vs.TCGA_vGCC_pro200"
dir.create(out.dir)

valid.mat <- read.table("RNA_preservation/For_preservation_msbb.BM_36.PMI_AOD_race_sex_RIN_exonicRate_rRnaRate_batch_adj.tsv", sep="\t", stringsAsFactors = FALSE, header = TRUE)

data.mat <- read.table("for_preservation/PHG_proteomics.allAdj_9209Value_with_uniqueID.txt", sep="\t", stringsAsFactors = FALSE, header = TRUE)
...
mol <- read.table("for_preservation/MEGENA_MSBB.PHG.proteomics.PMI_Age_race_sex_batch_adj_MEGENA2WGCNA_reformat00.tsv", sep="\t", stringsAsFactors = FALSE, header = TRUE)
mol00 <- mol[, c(2,4)]
