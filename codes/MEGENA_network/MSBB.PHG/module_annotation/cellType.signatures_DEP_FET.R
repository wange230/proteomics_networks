...
tbl <- read.table("signature_cnscelltype_only.txt", sep="\t", stringsAsFactors = F, header = T)
head(tbl)
...
write.table(enTable, file = "hyperGeTest_DEP/EnrichmentTable_DEP_final_FC11_cellType.signature.txt",
            sep="\t", row.names = FALSE)
