...
#removing 'NA'
info <- meta[!is.na(meta$PlaqueMean_final) == TRUE,]
mat <- mat[,!is.na(meta$CERAD_final) == TRUE] 
table(info$SynapseBrainID_final == colnames(mat)) # should all TRUE
info$pheno <- ifelse( info$PlaqueMean_final > 9, "High",  
                      ifelse (info$PlaqueMean_final > 0  ,"Medium", "Low" ) )
...
#write.table(ml, file="DEProtein_final_11_12_2019/Protein_exprCorrected_MvsL_PlaqueMean_final.tsv", sep="\t")
hm<- topTable(fit2, coef="High - Medium", number=Inf)
#write.table(hm, file="DEProtein_final_11_12_2019/Protein_exprCorrected_HvsM_PlaqueMean_final.tsv", sep="\t")
hl<- topTable(fit2, coef="High - Low",  number=Inf)
#write.table(hl, file="DEProtein_final_11_12_2019/Protein_exprCorrected_HvsL_PlaqueMean_final.tsv", sep="\t")
# over
