...
#removing 'NA'
info <- meta[!is.na(meta$CERAD_final) == TRUE,]
mat <- mat[,!is.na(meta$CERAD_final) == TRUE] 
table(info$SynapseBrainID_final == colnames(mat)) # should all TRUE
info$pheno <- ifelse( info$CERAD_final == "defAD", "High",  
                      ifelse (info$CERAD_final == "NL" , "Low" , "Medium") )
...
#write.table(ml, file="DEProtein_final_11_12_2019/Protein_exprCorrected_MvsL_CERAD_final.tsv", sep="\t")
hm<- topTable(fit2, coef="High - Medium", number=Inf)
#write.table(hm, file="DEProtein_final_11_12_2019/Protein_exprCorrected_HvsM_CERAD_final.tsv", sep="\t")
hl<- topTable(fit2, coef="High - Low",  number=Inf)
#write.table(hl, file="DEProtein_final_11_12_2019/Protein_exprCorrected_HvsL_CERAD_final.tsv", sep="\t")
# over
