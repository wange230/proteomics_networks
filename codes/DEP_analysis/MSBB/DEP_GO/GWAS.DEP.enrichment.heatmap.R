...
go <- read.table("EnrichmentTable_PHG.proteomics_abeta_ADGWAS.txt",
...
row.names(dff)[2] = "AD GWAS"
datt <- melt(dff)
datt$GWAS <- rep(row.names(dff), dim(go.sel)[1]/2)
head(datt)
...
pdf(file = "plot/PHG.prots.DEP_enrichemnt_over_GWAS_06_17_2024.pdf",
      width = 10.5, height = 3.5)
ggplot(datt, aes(variable, GWAS)) +
  geom_tile(aes(fill = `-log10(p.adj)`), colour = "white") +
  scale_fill_gradient(low = "white", high = "red") +
  labs(x = "", y = "")  +
  theme(axis.text.x = element_text(colour="grey20",
                                   size=9,angle= 45,hjust=1.0,vjust=1.0,face="plain"), #original 2
        axis.text.y = element_text(colour="grey20",size=9,angle=0,hjust=1,vjust=0,face="plain"),  
        axis.title.x = element_text(colour="grey20",size=12,angle=0,hjust=.5,vjust=0,face="plain"),
        axis.title.y = element_text(colour="grey20",size=12,angle=90,hjust=.5,vjust=.5,face="plain"), #original 4
